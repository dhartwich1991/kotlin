...
fun classpathFromClassloader(classLoader: ClassLoader): List<File>? =
        generateSequence(classLoader) { it.parent }.toList().flatMap {
            (it as? URLClassLoader)?.urLs?.mapNotNull {
                ...
                it.toFile()?.takeIf { el -> el.isDirectory || validJarExtensions.any { el.extension == it } }
            }
            ?: emptyList()
        }
...
object KotlinJars {
    ...
    val compilerClasspath: List<File> by lazy {
        ...
        val classpath = explicitCompilerClasspath
                        ...
                        ?: (classpathFromClass(Thread.currentThread().contextClassLoader, K2JVMCompiler::class)
                            ?: classpathFromClassloader(Thread.currentThread().contextClassLoader)?.takeIfContainsAll(PathUtil.KOTLIN_COMPILER_JAR)
                            ?: classpathFromClasspathProperty()?.takeIfContainsAll(PathUtil.KOTLIN_COMPILER_JAR)
    ...
    val stdlib: File? by lazy { getLib(KOTLIN_RUNTIME_JAR_PROPERTY, PathUtil.KOTLIN_JAVA_STDLIB_JAR, JvmStatic::class) }
